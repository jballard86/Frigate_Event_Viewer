...
│       ├── FrigateFirebaseMessagingService.kt   # FirebaseMessagingService: onNewToken → registerToken; onMessageReceived parses EventNotification; notification image from b64_thumb (NEW), image_url or FCM payload paths (live_frame_proxy, hosted_snapshot, notification_gif); handleNew, handleSnapshotReady, handleClipReady
...
└── ui/
    ...
    │   ├── EventsScreen.kt            # Events list: three modes (Reviewed/Unreviewed/Saved), full-width dropdown; optional camera filter UI when Settings showEventsCameraFilter is on (default off; All or per-camera via GET /events/&lt;camera&gt;; camera names from GET /cameras merged with event data, reserved segments like "events" excluded); Mark all reviewed (Unreviewed); **swipe-to-triage** on each card (Material3 SwipeToDismissBox): start→end = Mark reviewed (Unreviewed only) with Snackbar + Undo (DELETE /viewed); end→start = Keep (Unreviewed/Reviewed, not when event.saved); Saved mode / saved events: swipes disabled; haptic at half-dismiss; tap still opens detail; dynamic title; event cards show two-line GenAI title (fallback camera), 12h time, threat-level card tint (0 default, 1 yellow, 2 red); filters by UnreadState.locallyMarkedReviewedEventIds for Unreviewed; EventsViewModel activity-scoped (MainActivity), filter + camera in SavedStateHandle via CreationExtras; swipeMarkReviewed / swipeKeepEvent / undoMarkReviewed; LazyColumn uses stable key (event_id) and EventCardItem display model for list performance; 5m refresh throttle; watchdog prunes UnreadState
    │   ├── LiveScreen.kt              # Live tab: Select Camera dropdown from shared Go2RtcStreamsRepository state, preselects default from Settings when in list; live video player (stream URL: api/go2rtc/api/stream.mp4 via Frigate proxy 5000 only; original aspect ratio, fillMaxWidth, 12.dp; ExoPlayer with low-latency LoadControl and VIDEO_SCALING_MODE_SCALE_TO_FIT; Connecting/Loading status; error below player shows exact reason e.g. HTTP 404); bottom overlay “Picture in picture” button (Daily Review–style primaryContainer) when stream is ready; PiP: pause on ON_PAUSE only when not in PiP, hide controls in PiP; LiveViewModel/Factory (activity-scoped, created in MainActivity, passed via MainTabsScreen)
    │   ├── MainTabsScreen.kt          # HorizontalPager + bottom navigation hosting Live/Dashboard/Events/DailyReview; LaunchedEffect(selectedTabIndex) syncs pager when tab changes programmatically; header title from settledPage (no mid-swipe flicker); Snooze (Dashboard only) + Settings; tab index from MainTabsViewModel (SavedStateHandle) for rotation; default tab Dashboard (index 1); landscape: bottom bar hidden by default, AnimatedVisibility (expand/shrink) for show/hide; floating drag handle (circle+chevron, zIndex+offset) when bar closed; handle inside bottomBar Column above NavigationBar when bar open (transparent strip); alpha from SettingsPreferences; when isInPipMode: hide top bar and bottom nav, zero pager padding; landscape drag handle hidden in PiP
...
7. **FCM phase-aware notifications**
   - `FrigateFirebaseMessagingService.onMessageReceived` parses FCM data into `EventNotification`; if base URL is missing, no notification is posted. Notifications use `notificationId(ce_id)` so the same event updates in place. For the notification image, the app first checks [NotificationImageCache](app/src/main/java/com/example/frigateeventviewer/data/push/NotificationImageCache.kt) (72h TTL; evicted on get if expired). On cache miss, for **NEW** phase the backend may send `b64_thumb` (Base64 micro-thumbnail) in the data payload; when present the app decodes it for the large icon to avoid network latency, then falls back to image_url and live_frame_proxy/cropped_image_url if missing or decode fails. Otherwise, if FCM data includes `image_url` (full public URL), the app loads from that first; otherwise it loads from phase-specific FCM payload paths via buildMediaUrl (live_frame_proxy for NEW, hosted_snapshot for SNAPSHOT_READY, notification_gif for CLIP_READY, with cropped_image_url as fallback where used). Loaded/scaled bitmaps are cached by ce_id. No GET /events call or artificial delay is used for the notification image. On event delete, [EventDetailViewModel](app/src/main/java/com/example/frigateeventviewer/ui/screens/EventDetailViewModel.kt) calls `NotificationImageCache.removeForEventPath(eventPath)` so the cache does not retain the image. **NEW:** large icon only; alerts normally. **SNAPSHOT_READY:** applies initial AI title/description from payload (setContentTitle/setContentText and BigPictureStyle setBigContentTitle/setSummaryText); uses `setOnlyAlertOnce(true)` and `setDefaults(0)` for silent in-place update. **FINALIZED:** updates the same slot with final multi-clip AI title/description; image from NotificationImageCache only (no network); `setOnlyAlertOnce(true)` and `setDefaults(0)` for silent update; no Play/Keep/Mark Reviewed actions. **CLIP_READY:** AI title/description, Play action, Mark Reviewed and Keep actions (via [NotificationActionReceiver](app/src/main/java/com/example/frigateeventviewer/data/push/NotificationActionReceiver.kt)), large icon/GIF; `setOnlyAlertOnce(true)` and `setDefaults(0)` so the final update does not re-alert. FCM paths that look like absolute are normalized. Media URLs built with `buildMediaUrl(baseUrl, path)`; image loads use Coil with `allowHardware(false)` on the service's IO scope.
...
Navigation (routes, start destination, launch decision, bottom bar) is documented in `docs/UI_MAP.md`. The NavHost start destination is `"main_tabs"`; Settings is reached via the header settings icon (or from the deep-link flow when no base URL is set). Keep that document updated when adding or changing screens or routes. The Events screen has three filter states (Reviewed / Unreviewed / Saved) and a dynamic title per UI_MAP. Tab selection is preserved across configuration changes (e.g. rotation) via **MainTabsViewModel** (activity-scoped) and **SavedStateHandle**; the selected tab index is stored in saved state, so it survives Activity recreation regardless of NavHost. In landscape the bottom tab bar is hidden by default; show/hide is animated (expand/shrink). A semi-transparent drag handle (circle + chevron, 50% opacity) floats at bottom-right when the bar is closed; when the bar is open the handle sits inside the bottom bar above the NavigationBar.gationBar. Drag up to show, drag down to hide.
